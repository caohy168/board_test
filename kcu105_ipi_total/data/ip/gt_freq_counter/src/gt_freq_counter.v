...
IBUFDS_GTE3 IBUFDS_GTE3_inst_DIFF_MGTE3_CLK1(
   .O (), // Buffer output
   .ODIV2(DIFF_MGTE3_out),
   .I (DIFF_MGTE3_P), // Diff_p buffer input (connect directly to top-level port)
   .IB(DIFF_MGTE3_N)  // Diff_n buffer input (connect directly to top-level port)
);
BUFG_GT BUFG_GT_inst_DIFF_MGTE3_CLK1 (
      .O(DIFF_MGTE3_bufg), // 1-bit output: Clock output
      .I(DIFF_MGTE3_out)
      );

assign TEST_CLK_I = DIFF_MGTE3_bufg;
